 ...
 * @return 
**/
bool CasillaTransitable(const ubicacion & x, const vector<vector<unsigned char> > & map){
...
/**
 * @brief Encuentra si el elemento item está en la lista
 * @param item 
 * @param lista 
 * @return 
 */
bool Find(const stateN0 &item, const list<stateN0> &lista){
	auto it = lista.begin();
	while (it != lista.end() && !((*it) == item)) it++;
    // find(lista.begin(), lista.end(), item);
	return (!(it == lista.end()));
}

/**
 * @brief Primera aproximación a la implementación de la búsqueda en anchura
 * @param inicio 
 * @param final 
 * @param mapa 
 * @return 
...
/**
 * @brief Sobrecarga de la función Find para buscar en lista de nodeN0
 * @param item 
 * @param lista 
 * @return 
 */
...
/**
 * @brief Segunda aproximación a la implementación de la búsqueda en anchura
 * @param inicio 
 * @param final 
 * @param mapa 
 * @return 
...
Action ComportamientoJugador::think(Sensores sensores)
{
	...
	if (sensores.nivel != 4)
    {
        if (!hayPlan)
        {
            ...
            //plan = AnchuraSoloJugador_V2(current_state, goal, mapaResultado);

            switch (sensores.nivel)
            {
                case 0:
                    plan = AnchuraSoloJugador_V3(current_state, goal, mapaResultado);
                break;
                case 1:
                break;
                case 2:
